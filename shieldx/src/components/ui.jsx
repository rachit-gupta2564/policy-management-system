  ...
  return (
    ...
    >
      <div className={`bg-white rounded-2xl shadow-[0_24px_80px_rgba(0,0,0,0.25)] w-full max-h-[90vh] overflow-y-auto animate-modal-in ${wide ? 'max-w-2xl' : 'max-w-lg'}`}>
        ...
        {footer && (
          <div className="px-7 py-4 border-t border-gray-100 flex items-center justify-end gap-3">{footer}</div>
        )}
      </div>
    </div>
  )
...
export function Textarea(props) {
  return (
    <textarea {...props}
      className="w-full border-[1.5px] border-gray-200 rounded-xl px-4 py-3 text-sm font-sans text-gray-900 bg-white outline-none focus:border-brand focus:ring-2 focus:ring-brand/10 min-h-[100px] resize-y"
    />
  )
}
...
export function BtnBrand({ children, onClick, className = '' }) {
  return (
    <button onClick={onClick}
      className={`bg-brand hover:bg-brand-light text-white font-semibold py-3 px-6 rounded-xl transition-all duration-150 hover:-translate-y-px ${className}`}
    >{children}</button>
  )
}
...
export function BtnGhost({ children, onClick, className = '' }) {
  return (
    <button onClick={onClick}
      className={`bg-surface border border-gray-200 text-gray-500 font-medium py-2.5 px-5 rounded-xl hover:bg-gray-100 transition-colors ${className}`}
    >{children}</button>
  )
}
...
export function BtnSm({ children, onClick, variant = 'blue' }) {
  ...
  return (
    <button onClick={onClick}
      className={`px-3 py-1.5 rounded-lg text-xs font-bold transition-colors ${v[variant]}`}
    >{children}</button>
  )
}

export function Card({ children, className = '' }) {
  return <div className={`bg-white border border-gray-100 rounded-2xl shadow-card ${className}`}>{children}</div>
}
      ...
      <table className="w-full border-collapse">
        <thead>
          <tr className="bg-surface">
            {headers.map(h => (
              <th key={h} className="text-left px-6 py-3 text-[11px] font-semibold uppercase tracking-widest text-gray-400 border-b border-gray-100">{h}</th>
            ))}
          </tr>
        </thead>
...
export function UploadZone({ label = 'Click to upload', sub = 'PDF, JPG, PNG (max 5MB)', onClick }) {
  return (
    ...
    >
      ...
      <div className="text-sm text-gray-500"><strong className="text-brand">{label}</strong> or drag and drop</div>
...
export function SectionHeader({ eyebrow, title, sub }) {
  return (
    <div className="mb-10">
      {eyebrow && <div className="text-xs font-semibold uppercase tracking-widest text-brand-light mb-2">{eyebrow}</div>}
...
export function StatCard({ label, value, change, changeType = 'neutral' }) {
  const c = changeType === 'up' ? 'text-green-600' : changeType === 'down' ? 'text-red-500' : 'text-gray-400'
  return (
    <Card className="p-5">
      ...
      {change && <div className={`text-xs mt-1.5 ${c}`}>{change}</div>}
    </Card>
  )
}